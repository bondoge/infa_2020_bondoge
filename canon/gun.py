...
while not finished:
    ...
    textSurface = font.render('points:' + str(target.points), False, (0, 0, 0))
    ...
    for b in balls:
        b.move()
        if b.hittest(target) and target.live and gun.shots > 0:
            target.live = 0
            target.hit()
            if gun.shots == 1: s = ' shot'
            else: s = ' shots'
            textSurface2 = font.render('You\'ve destroyed the target in ' + str(gun.shots) + s, False, BLACK)
            gun.shots = 0
